/**
 * BFF Endpoint: GET /api/categories
 * Get all categories
 */
import { NextRequest, NextResponse } from "next/server"
import { CategoriesResponseSchema } from "@/lib/schemas/product.schema"
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 2. Call backend
    const backendUrl = `${API_BASE_URL}${API_ENDPOINTS.categories.list}`
    ...
    const responseValidation = CategoriesResponseSchema.safeParse(backendData)
